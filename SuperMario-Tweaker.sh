#!/system/bin/sh
# SuperMario Tweaker Script - Exynos
...
# Set small CPU cores (0-3) to fixed frequency on Exynos
for cpu in 0 1 2 3; do
    cpu_path="/sys/devices/system/cpu/cpu$cpu/cpufreq"

    if [ -d "$cpu_path" ]; then
        if echo schedutil > "$cpu_path/scaling_governor"; then
            log_tweak "Set CPU core $cpu to schedutil governor" 1
        else
            log_tweak "Failed to set governor for CPU core $cpu" 0
        fi

        if echo 1500000 > "$cpu_path/scaling_min_freq"; then
            log_tweak "Set min frequency for CPU core $cpu to 1500000" 1
        else
            log_tweak "Failed to set min frequency for CPU core $cpu" 0
        fi

        if echo 1500000 > "$cpu_path/scaling_max_freq"; then
            log_tweak "Set max frequency for CPU core $cpu to 1500000" 1
        else
...
# Set big CPU cores (4-7) to fixed frequency on Exynos
for cpu in 4 5 6 7; do
    ...
    if [ -d "$cpu_path" ]; then
        ...
        if echo 2600000 > "$cpu_path/scaling_min_freq"; then
            log_tweak "Set min frequency for CPU core $cpu to 2600000" 1
        else
            log_tweak "Failed to set min frequency for CPU core $cpu" 0
        fi

        if echo 2600000 > "$cpu_path/scaling_max_freq"; then
            log_tweak "Set max frequency for CPU core $cpu to 2600000" 1
        else
            log_tweak "Failed to set max frequency for CPU core $cpu" 0
        fi
    fi
...
if echo 1 > /sys/module/exynos_thermal/core_control/enabled; then
...
if echo N > /sys/module/exynos_thermal/parameters/enabled; then
    log_tweak "Set exynos_thermal enabled to N" 1
else
    log_tweak "Failed to set exynos_thermal enabled" 0
fi

if echo 0 > /sys/module/exynos_thermal/core_control/enabled; then
    log_tweak "Disabled core control for exynos_thermal" 1
else
    log_tweak "Failed to disable core control for exynos_thermal" 0
fi

if echo 0 > /sys/kernel/exynos_thermal/enabled; then
    log_tweak "Disabled exynos_thermal" 1
else
    log_tweak "Failed to disable exynos_thermal" 0
fi
